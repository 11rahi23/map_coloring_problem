...
def degree_heuristics(item_list):
	heuristics={}
	count=0
	for item in item_list:
		...
		for neighbour in graph[item]:
			if assignment[neighbour]==None:
				heuristics[item]+=1
		print("item: ", item)
		print("heuristics_item: ", heuristics[item])
	heuristics=dict(sorted(heuristics.items(), key=lambda item:item[1]))
	print("sorted heuristics: ", heuristics)
	var=list(heuristics.keys())[-1]
	print("var: ", var)
	return var
...
def select_unassigned_variable(assignment):
	...
	var=list(counter.keys())[0]											##apparently here MRV and forward checking is applied together
	...
	for item in counter:
		print("Item: ", item)
		if counter[item]==min_value[1]:
			heur.append(item)
			count+=1
	print("heur: ", heur)
	if len(heur)>1:
		var=degree_heuristics(heur)
